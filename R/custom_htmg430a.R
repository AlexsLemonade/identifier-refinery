...
cel_path <- "/home/user/data/GSM516950.CEL"
platform <- "htmg430a"
org_code <- "Hs"
...
ba_package_name <- paste0(platform, "probe")
...
gpl_df <- readr::read_tsv("/home/user/data/GPL8759-17365.txt", comment = "#") %>%
  dplyr::select(c("ID", "Gene Symbol", "Entrez Gene", "RefSeq Transcript ID")) %>%
...
output_file <- file.path("/home/user/data/out", paste0("htmg430a_master_conversion.tsv.gz"))
